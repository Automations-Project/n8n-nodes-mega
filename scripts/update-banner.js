...
function resolveNodeLogoUrl(root, owner, repo, iconDirRelative) {
  // Prefer the svg next to the node icon; default to nodes/Mega/mega.svg
  let localSvg = '';
  if (iconDirRelative) {
    const candidate = path.join(root, iconDirRelative, 'mega.svg');
    if (fs.existsSync(candidate)) localSvg = candidate;
  }
  if (!localSvg) {
    const fallback = path.join(root, 'nodes', 'Mega', 'mega.svg');
